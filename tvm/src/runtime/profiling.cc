...
PackedFunc WrapTimeEvaluator(
    ...
    auto ftimer = [=](TVMArgs args, TVMRetValue* rv) mutable {
...
#ifdef TVM_ENABLE_NVML_POWER
        auto t_est_start = std::chrono::steady_clock::now(); // 측정 시에만 시간 기록
#endif

        pf.CallPacked(args, &temp);           // 커널 실행 (항상 수행)
        DeviceAPI::Get(dev)->StreamSync(dev, nullptr); // 동기화 (항상 수행)
        ...
        // --- [STEP 1] Accurate Reactive Warmup ---
        {
            std::lock_guard<std::mutex> lock(power_mtx);
            power_buffer.clear();
        }
        ...
        // [핵심 변경] 커널 무게(Latency)에 따른 이원화 설정
        int window_size;
        int required_stable_steps;
        double stability_threshold;

        if (est_lat_ms > 100.0) { 
            // 무거운 커널 (예: Llama 3.1) - 단 몇 번의 실행으로 즉시 안정화 판단
            // 커널 1개 실행 시간만큼만 윈도우를 잡음 (5ms 단위)
            window_size = std::max(20, (int)(est_lat_ms / 5.0)); 
            required_stable_steps = 2;   // 단 2번만 만족해도 즉시 탈출 (너무 오래 끌 필요 없음)
            stability_threshold = 0.05;  // LLM 특유의 전력 스파이크를 고려해 5% 오차 허용
        } else { 
            // 가벼운 커널 (예: DenseNet) - 기존 기준 유지
            window_size = 100;           
            required_stable_steps = 10;  
            stability_threshold = 0.01;  // 1% 오차
        }

        const int max_warmup_ms = 30000;

        // 체크 한 번당 약 25ms 분량의 부하를 주되, 무거운 커널은 무조건 1번씩만 실행
        int warmup_batch = (est_lat_ms > 25.0) ? 1 : std::max(1, (int)(25.0 / est_lat_ms));
        ...
        while (true) {
...

            {
                ...
                if (power_buffer.size() >= (size_t)(window_size + 1)) {
                    ...
                    // 최근 5개(25ms) 샘플의 평균을 현재 값으로 사용하여 노이즈 상쇄
                    double sum_recent = 0;
                    int recent_count = std::min(5, (int)power_buffer.size());
                    for (size_t i = power_buffer.size() - recent_count; i < power_buffer.size(); ++i) {
                        sum_recent += power_buffer[i];
                    }
                    double current_val = sum_recent / recent_count;
                    final_warmup_power = current_val;
                    
                    if (current_val > max_power_seen) max_power_seen = current_val;

                    // 이전 window_size 구간의 평균
                    double sum_window = 0;
                    for (size_t i = power_buffer.size() - (window_size + recent_count); i < power_buffer.size() - recent_count; ++i) {
                    ...
                    // 상승 억제(<= 0.001) 같은 까다로운 조건 제거, 단순 허용 오차만 확인
                    bool is_flat = (std::abs(diff_ratio) < stability_threshold);
                    
                    // 피크 기준도 95%로 완화 (Heavy 커널은 피크를 찍고 약간 내려와서 유지되는 경향이 있음)
                    bool is_at_peak = (current_val >= max_power_seen * 0.95);
